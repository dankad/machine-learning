...
public class LinearRegression implements Classifier {
	...
	public void buildClassifier(Instances trainingData) throws Exception {
		...
		// run linyer regretion with correct alpha
		m_coefficients = m_firstTeta;//return to starting point
		
		double currentError = Integer.MAX_VALUE - 1;
		double prevError = Integer.MAX_VALUE;
		
		while ( (prevError - currentError) > 0.003) {
	...
	public double calculateMSE(Instances data) throws Exception {
		...
		for (int i = 0; i < data.size(); i++) {
			double predection = this.regressionPrediction(data.instance(i));
